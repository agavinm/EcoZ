...
function guardarDiscurre(rutaVO, zonasVOList) {
  ...
  for (var i = 0; i < zonasVOList.length && !error; i++) {
    $.ajax({
      ...
      data: {rutaId: rutaVO.id, zonaNombre: zonasVOList[i].nombre},
      success: function(discurreVO) {
        if (discurreVO.hasOwnProperty('error')) {
          error = true;
          alert(discurreVO.error);
        }
      },
  ...
  $.ajax({
    ...
    success: function(rutaVO) {
      if (rutaVO.hasOwnProperty('error')) {
        ...
        guardarDiscurre(rutaVO, zonesOfRoute[numRuta-1]);
      }
    },
